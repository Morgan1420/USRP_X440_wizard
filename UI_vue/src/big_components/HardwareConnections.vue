  ...
  <div class="hardware-shell" ref="containerRef">
    ...
    <section class="ports-section">
      ...
      <div class="ports-box">
        ...
        <div v-for="i in 4" :key="'left-'+i" class="port" :ref="el => setPortRef(el, i-1)" :class="{assigned: portToPartial[i-1] != null}" @pointerdown.prevent="startDragFromPort(i-1, $event)">{{ portLabels[i-1] }}</div>
        <div class="empty-port"></div>
        <div v-for="i in 4" :key="'right-'+i" class="port" :ref="el => setPortRef(el, i+3)" :class="{assigned: portToPartial[i+3] != null}" @pointerdown.prevent="startDragFromPort(i+3, $event)">{{ portLabels[i+3] }}</div>
        <div class="empty-port"></div>
      </div>
    ...
    <div class="footer">
      <Button label="Auto-connect" @click="autoAssign"/>
...
const numPorts = props.numPorts
const portLabels = Array.from({ length: numPorts }, (_, i) => `P${i+1}`)
const portsSelected = ref(Array.from({ length: numPorts }, () => false))
...
const zoneBoxes = computed(() => {
  // if backend provided zone_boxes, use it (already grouped); otherwise try to create simple zones
  if (props.option && props.option.zone_boxes) return props.option.zone_boxes
  // fallback: build single zone from partials
...
function assignPortToPartial(portIdx, pi, si){
  ...
  if (partials.value.length === 2) {
    if (pi === 0 && portIdx >= Math.floor(numPorts/2)){
      alert('Partial 1 can only connect to the first 4 ports')
      return
    }
    if (pi === 1 && portIdx < Math.floor(numPorts/2)){
      alert('Partial 2 can only connect to the last 4 ports')
      return
    }
  }
  if (prev === key) return

  // simulate removal from previous and ensure previous partial remains connected
  if (prev != null){
    const arrPrev = (partialToPorts.value[prev] || []).filter(x => x !== portIdx)
    if (arrPrev.length === 0){
      // try to find an unassigned physical port to give to prev
      const unassigned = Array.from({ length: numPorts }).find((_, idx) => portToPartial.value[idx] == null && idx !== portIdx)
      if (unassigned != null){
        portToPartial.value[unassigned] = prev
        partialToPorts.value[prev] = [unassigned]
      } else {
        ...
        for (let j = 0; j < numPorts; j++){
          ...
          if (pj && (partialToPorts.value[pj] || []).length > 1){
            // remove j from pj and give to prev
        ...
        if (stolen == null){
          alert('Cannot reassign: previous partial would become unconnected and no spare ports available.')
          return
        }
      }
    } else {
...
function removeConnection(portIdx){
  ...
  if (arr.length <= 1){
    // cannot remove last connection for a partial
    alert('Cannot remove last connection for this partial port.')
    return
  }
...
function onPortToggle(i, isSelected){
  // i is index, isSelected boolean
  if (selectedBox.value != null && isSelected){
    // assign port i to selectedBox
...
function onNetworkUpdate(cfg){
  // forward network configuration if needed
  emit('network-update', cfg)
}

function autoAssign(){
  // simple one-to-one assignment for first N boxes
  const partialList = flattenPartialPorts()
  portToPartial.value = {}
  partialToPorts.value = {}
  const plist = flattenPartialPorts()
  if (partials.value.length === 2) {
    // split physical ports: first half -> partial 0, second half -> partial 1
    const leftPorts = Array.from({ length: Math.floor(numPorts/2) }, (_, i) => i)
    const rightPorts = Array.from({ length: Math.ceil(numPorts/2) }, (_, i) => i + Math.floor(numPorts/2))
    // assign partial 0 ports to leftPorts
    ...
    // assign partial 1 ports to rightPorts
    ...
    // distribute any remaining physical ports round-robin to partials while respecting halves
    let li = p0.length
    ...
    // default round-robin assignment ensuring each partial-port has at least one physical port
    const pcount = plist.length
    let assigned = 0
    for (let i = 0; i < Math.min(numPorts, pcount); i++){
    ...
    for (let i = assigned; i < numPorts; i++){
...
.dot-port{ width:12px; height:12px; border-radius:50%; background:#2b80ff; border:2px solid #164fdd }
...
.ports-section, .partials-section, .network-section{ background:#fff; border:1px solid #eee; padding:12px; border-radius:6px }
...
.port.assigned{ box-shadow:0 0 0 3px rgba(34,197,94,0.12) }
</style>
